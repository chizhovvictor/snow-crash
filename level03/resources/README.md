Основная задача, я так понимаю всех, упрожнений выполнить getflag под нужным пользователем. В таком случае getflag вернет нам токен который мы сможем использовать для футентификации.
